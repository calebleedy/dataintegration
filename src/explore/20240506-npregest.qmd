...
gen_pop <- function(N_obs, n1_obs) {
  ...
  z <- rnorm(N_obs)
  eps <- rnorm(N_obs)
  # y <- 0.05 * x^2 + 2 * eps + exp(x - max(x) + 4) + 2 * sin(x)
  y <- x + z + eps

  pi1 <- n1_obs / N_obs
  pi2 <- ifelse(pnorm(z - 1) > 0.7, 0.7, pnorm(z - 1))
  pi2 <- ifelse(pi2 < 0.02, 0.02, pi2)

  w1 <- 1 / pi1
  w2 <- 1 / pi2

  return(tibble(X = x, Y = y, pi1 = pi1, pi2 = pi2, w1 = w1, w2 = w2))
...
naive_npreg_est <- function(p1_df, p2_df, N_obs, type = "loess") {
  ...
  # 1. Estimate mhat.
  if (type == "loess") {
    mod <- loess(Y ~ X, data = p2_df, weights = w2,
                 control = loess.control(surface="direct"))
  } else if (type == "randomforest") {
    mod <- randomForest(Y ~ X, data = p2_df)
  } else if (type == "spline") {
    mod <- gam(Y ~ s(X, k = 15, bs = "cr"), data = p2_df, weights = w2)
  } else if (type == "linear") {
    mod <- lm(Y ~ X, data = p2_df, weights = w2)
  }
...
ss_npreg_est <- function(p1_df, p2_df, N_obs, type = "loess") {
  ...
  p1_df <- p1_df %>%
    left_join(p2_df, by = join_by(X, Y, pi1, pi2, w1, w2, del1, del2)) %>%
    mutate(del2a = ifelse(is.na(del2a), 0, del2a)) %>%
    mutate(del2b = ifelse(is.na(del2b), 0, del2b))

  # 1. Estimate mhat in both splits
  if (type == "loess") {
    moda <- loess(Y ~ X, data = p2a_df, weights = w2,
                  control = loess.control(surface="direct"))
    modb <- loess(Y ~ X, data = p2b_df, weights = w2,
                  control = loess.control(surface="direct"))
  } else if (type == "randomforest") {
    moda <- randomForest(Y ~ X, data = p2a_df)
    modb <- randomForest(Y ~ X, data = p2b_df)
  } else if (type == "spline") {
    moda <- gam(Y ~ s(X, k = 15, bs = "cr"), data = p2a_df, weights = w2)
    modb <- gam(Y ~ s(X, k = 15, bs = "cr"), data = p2b_df, weights = w2)
  } else if (type == "linear") {
    moda <- lm(Y ~ X, data = p2a_df, weights = w2)
    modb <- lm(Y ~ X, data = p2b_df, weights = w2)
  }
...
mc_res <- 
  foreach(iter=1:B_sims,
          .packages = c("dplyr", "randomForest", "mgcv"),
          .options.RNG  = 2) %dorng% {
