...
acs_cl <- read_csv("../build/cleaned_acs_data.csv")
...
data_int_mcbsacs <- function(mcbs_all, cov_ests) {
  ...
  # d_vec <- 1 / p0_df$pi0
  d_vec <- mcbs_all$PUFSWGT
  if (entropy == "EL") {
    gdi <- -1 / d_vec
    # FIXME: We need an estimate of gdip
    gdip <- -1 / (1 / pop_df$pi0)
    # END FIXME:
    gpinv <- d_vec^2
  } else if (entropy == "ET") {
  ...
  samp_X <- model.matrix(~edulths + edueqhs + 
                         marmar + marwid + mardiv + 
                         sexm + racew + raceb + raceh, data = mcbs_all)
  zmat <- cbind(samp_X, gdi)
  w1i <- 1

  c_ests <- filter(cov_ests, !(Name %in% c("edumths", "sexf", "marsin")))
  xhat <- c_ests$Est
  ...
  x_mat <- matrix(c(as.numeric(c_ests$Name == "tot"),
                    ...
                    as.numeric(c_ests$Name == "racew"),
                    as.numeric(c_ests$Name == "raceb"),
                    as.numeric(c_ests$Name == "raceh")),
                  ncol = 10)

  vmat <- diag(c_ests$Var)
  vinv <- diag(1 / c_ests$Var)
  xgls <- drop(solve(t(x_mat) %*% vinv %*% x_mat) %*% 
              (t(x_mat) %*% vinv %*% xhat))
  T1 <- c(xgls, sum(gdip * qi))
  dc_w <- solveGE(zmat, w1i, d_vec, T1, qi, entropy)
  ...
  gam_dc <- 
    solve(t(z_dc) %*% diag(d_vec * gpinv * qi) %*% z_dc,
          t(z_dc) %*% diag(d_vec * gpinv) %*% Y_vec)
  coefs <- as.numeric(gam_dc)
  u_mat <- sampX
  eps <- (Y_vec - drop(u_mat %*% coefs))
