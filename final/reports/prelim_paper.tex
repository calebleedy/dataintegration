...
% Outline
% Instead of incorporating data from one additional source, we want to combine
% data from multiple sources
% * Use GLS (generalized from effective sample size)
% * We need to add debiasing constraint 
%   * What do we do if we have multiple surveys for which we observe Y?
%   * How do we incorporate the selection probability?
...
where $\hat T = (\hat{\bf X}^{(m)}, \sum_{i \in U} g(d_{im}) q_i)^T$ and $d_{im}$
