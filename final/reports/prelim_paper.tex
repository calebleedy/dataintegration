...
\begin{itemize}
  \item[1.] HT-estimator: $\hat Y_{HT} = N^{-1} \sum_{i \in A_2}
    \frac{y_i}{\pi_{2i}},$
  \item[2.] Regression estimator (Reg): Let $\hat Y_{NN, reg} = \hat Y_{HT} + (\hat
    X_c - \hat X_{2, HT}) \hat \beta_2$ where $\hat Y_{HT} = \sum_{i \in A_2} d_{2i}
    y_i$, $\hat X_c = W \hat X_{1, HT} + (1 - W)\hat X_{2, HT}$, $W = n_{1,eff}
    / (n_{1,eff} + n_{2, eff})$, $\hat X_{1, HT} = \sum_{i \in A_1} d_{1i} x_{i}$,
    $\hat X_{2, HT} = \sum_{i \in A_2} d_{2i} x_{i}$ and 

    $$ \hat \beta_2 = \left(\sum_{i \in A_2} x_i x_i^T\right)^{-1} \sum_{i \in
    A_2} x_i y_i.$$
